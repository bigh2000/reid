...
# 'market1501','cuhk03','dukemtmcreid','msmt17','viper','grid','cuhk01','prid450s','ilids'
...
parser.add_argument('-d', '--dataset', type=str, default=ds,
...
parser.add_argument('--seed', type=int, default=1,
                    help="manual seed")
parser.add_argument('--resume', type=str, default='saved-models/vdo-resnet50-xent-combine_best.pth.tar', metavar='PATH')
...
def main():
    ...
    name = args.resume.split('/')[1].split('.')[0]
    if not args.evaluate:
        sys.stdout = Logger(osp.join(args.save_dir, 'log-train-'+name+'.txt'))
    else:
        sys.stdout = Logger(osp.join(args.save_dir, 'log-test-'+name+'.txt'))
    print("==========\nArgs:{}\n==========".format(args))
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if (epoch+1) > args.start_eval and args.eval_step > 0 and (epoch+1) % args.eval_step == 0 or (epoch+1) == args.max_epoch:
            ...
            save_checkpoint({
                ...
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, name+'.pth.tar'))
